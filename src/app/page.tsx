...
export default function Home() {
  ...
  return (
      <div
          className="login w-full h-screen bg-base-100 grid grid-rows-[max-content_max-content_1fr] md:grid-rows-[initial] md:place-items-center md:grid-cols-[3fr_2fr]">
        <div className="w-full mt-10 flex justify-center p-5 md:pr-0 flex-wrap">
          ...
          >
            <h2 className="text-center text-2xl ">
              <span className="has-[+input:checked]:opacity-40">Sign Up</span>{" "}
              <input
                  type="checkbox"
                  style={{"--input-color": "var(--color-base-content)"} as React.CSSProperties}
                  className="toggle toggle-xl mx-5 peer"
                  checked={formState === "login"}
                  onChange={() => setFormState(formState === "login" ? "signup" : "login")}
              />
              {" "}
              <span className="peer-[:not(:checked)]:opacity-40">Log In</span>
            </h2>
